...
def test_pause_http_error(caplog: pytest.LogCaptureFixture) -> None:
    """Test pause operation with HTTP error during API call."""
    ...
    # Simulate error during the list comprehension
    mock_archiver.pause_pv.side_effect = http_error
    ...
    with (
        ...
        # pause_pv should have been called once before raising the error
        mock_archiver.pause_pv.assert_called_once_with("PV1")
        mock_validate_operation_results.assert_not_called()
        assert "Error pausing PVs" in caplog.text
        assert str(http_error) in caplog.text  # Check if the original error message is logged
        assert "Error pausing PVs." in caplog.text  # Check debug log message
...
def test_resume_http_error(caplog: pytest.LogCaptureFixture) -> None:
    """Test resume operation with HTTP error during API call."""
    caplog.set_level(logging.DEBUG)
    archiver_fqdn = "archiver.example.com"
    pvs = ["PV1"]
    mock_archiver_info = MagicMock(spec=ArchiverMgmtInfo)
    ...
    http_error = HTTPError(response=request_response)
    # Simulate error during the list comprehension
    mock_archiver.resume_pv.side_effect = http_error
    ...
    with (
        ...
        # resume_pv should have been called once before raising the error
        mock_archiver.resume_pv.assert_called_once_with("PV1")
        mock_validate_operation_results.assert_not_called()
        assert "Error resuming PVs" in caplog.text
        assert str(http_error) in caplog.text  # Check if the original error message is logged
        assert "Error resuming PVs." in caplog.text  # Check debug log message
        assert exc_info.value.__cause__ is http_error
